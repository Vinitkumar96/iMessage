...
export function ChatSidebar(){
    ...
    const conversation = useChatStore((state) => state.conversations);
    ...
    // mapping both users and conversation into strucuterd object
    const conversationUsers = conversation.map((user) => mapUserForList(user,onlineUsers));
    const allUsers = users.map((user) => mapUserForList(user,onlineUsers))

    //if any query we will filter conversationusers and allusers according to it
    const filteredConversations = normalizedSearchQuery ? conversationUsers.filter((conversation) => conversation.peer.name.toLowerCase().includes(normalizedSearchQuery)) : conversationUsers;
    const filteredUsers = normalizedSearchQuery ? allUsers.filter((user) => user.peer.name.toLowerCase().includes(normalizedSearchQuery)) :  allUsers;
